...
const productTitle = computed(() => cleanText(product.value?.title, "Вязаное изделие"));
const productDescription = computed(() => cleanText(product.value?.description, "Описание временно недоступно."));
      ...
      <section class="brand-card p-5 md:p-6">
        ...
        <div v-else-if="activeTab === 'reviews'" class="space-y-4">
          ...
          >
            ...
            <p class="mt-2 text-[1.07rem] text-muted">{{ cleanText(review.text, "Без текста") }}</p>
          </article>
        </div>
