...
def run_seed() -> None:
    ...
    with Session(engine) as db:
        ...
        db.add(SellerProfile(user_id=seller.id, display_name="������ ����", bio="����� �������� � ��������"))

        cat_ceramic = Category(name="��������", slug="ceramic")
        cat_textile = Category(name="��������", slug="textile")
        ...
        db.add(Review(user_id=buyer.id, product_id=p_active.id, order_item_id=order_item.id, rating=5, text="����� �������!"))
        ...
        db.add_all(
            [
                Message(conversation_id=conv.id, sender_id=buyer.id, body="������������, ����� ��������?", is_read=False),
                Message(conversation_id=conv.id, sender_id=seller.id, body="������ ����, ������ �����.", is_read=True),
            ]
        )
